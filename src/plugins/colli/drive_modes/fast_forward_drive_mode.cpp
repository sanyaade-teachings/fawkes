...
namespace fawkes
{
   ...
   */
  float
  CFastForwardDriveModule::FastForward_Curvature(float dist_to_target, float dist_to_trajec, float alpha, float trans_0,
      float rot_0)
  {
    return 1.5 * alpha;
  }
      ...
      float trans_0, float rot_0, float rot_1)
  {
    ...
    if (fabs(rot_1) >= 0.0 && fabs(rot_1) <= 0.3) {
      ...
      loggerFastFor->log_error("CFastForwardDriveModule",
          "************ FAST DRIVE MODES:::NOT DEFINED STATE!!!!! STOPPING\n");
      trans_1 = 0;
    }
  ...
  CFastForwardDriveModule::Update()
  {
    ...
    m_ProposedRotation = FastForward_Curvature(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot);

    if (fabs(alpha) > M_PI_2) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = FastForward_Translation(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot,
          m_ProposedRotation);
    }
