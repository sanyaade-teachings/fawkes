...
namespace fawkes
{
  ...
  CSelectDriveMode::CSelectDriveMode(MotorInterface* motor, Laser* laser, NavigatorInterface* target, Logger* logger,
...
//
    ...
    m_vDriveModeList.push_back(
        (CAbstractDriveMode *) new CSlowBiwardDriveModule(logger, config, slow_forward, slow_backward));
    ...
    m_vDriveModeList.push_back(
        (CAbstractDriveMode *) new CMediumBiwardDriveModule(logger, config, medium_forward, medium_backward));
    ...
    m_vDriveModeList.push_back(
        (CAbstractDriveMode *) new CFastBiwardDriveModule(logger, config, fast_forward, fast_backward));
  ...
  CSelectDriveMode::Update(bool escape)
  {
    ...
    desiredMode = (ColliModes) (m_pColliTarget->drive_mode());
...
//    loggerSelect->log_info("select drive","colli mode is: %s",desiredMode);
     ...
     */
    if (escape == true) {
      if (m_EscapeFlag == 0 && m_pMotor->vx() != 0 && m_pMotor->omega() != 0) {
        desiredMode = MovingNotAllowed; // we have not yet stopped!
      } else {
        m_EscapeFlag = 1;               // we have stopped recently, so do escape!
        desiredMode = ESCAPE;
    ...
    for (unsigned int i = 0; i < m_vDriveModeList.size(); i++) {
      // error checking
      if (m_vDriveModeList[i]->GetDriveModeName() == desiredMode && m_pDriveMode != 0) {
        loggerSelect->log_info("CSelectDriveMode",
            "Error while selecting drive mode. There is more than one mode with the same name!!!\nStopping!\n");
    ...
    } else  // valid drive mode!
    {
      ...
      m_pDriveMode->SetCurrentRoboSpeed(GetMotorTranslation(m_pMotor->vx(), m_pMotor->omega()), m_pMotor->omega());

      m_pDriveMode->SetCurrentTarget(m_pColliTarget->dest_x(), m_pColliTarget->dest_y(), m_pColliTarget->dest_ori());
  ...
  float
  CSelectDriveMode::GetMotorTranslation(float vtrans, float vori)
  {
  ...
  float
  CSelectDriveMode::GetMotorOri(float odom_ori)
  {
