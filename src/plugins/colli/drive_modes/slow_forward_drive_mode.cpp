...
//    cout << "CSlowDriveModule_MAX_ROT: " << m_MaxRotation << endl;
   ...
   */
  float
  CSlowForwardDriveModule::SlowForward_Curvature(float dist_to_target, float dist_to_trajec, float alpha, float trans_0,
      float rot_0)
  {
    return 1.2 * alpha;
  }
  ...
  CSlowForwardDriveModule::Update()
  {
    ...
    m_ProposedRotation = SlowForward_Curvature(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot);

    if (fabs(alpha) > M_PI_2 + 0.1) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = SlowForward_Translation(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot,
          m_ProposedRotation);
    }
