...
namespace fawkes
{
      ...
      float trans_0, float rot_0, float rot_1)
  {
    ...
    if (fabs(rot_1) >= 0.0 && fabs(rot_1) <= 0.5) trans_1 = LinInterpol(fabs(rot_1), 0.5, 0.0, 1.7,
        m_MaxTranslation + 0.05);

    else if (fabs(rot_1) > 0.5 && fabs(rot_1) <= 1.0) trans_1 = LinInterpol(fabs(rot_1), 1.0, 0.5, 1.2, 1.7);
      ...
      loggerMedFor->log_error("CMediumForwardDriveModule",
...
//  float dist_to_trajec  = sqrt(  pow((m_LocalTrajecX),2) + pow((m_LocalTrajecY),2) );
    ...
    m_ProposedRotation = MediumForward_Curvature(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot);

    if (fabs(alpha) > M_PI_2 + 0.2) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = MediumForward_Translation(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot,
          m_ProposedRotation);
    }
