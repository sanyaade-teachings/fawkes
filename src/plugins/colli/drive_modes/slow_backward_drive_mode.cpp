...
//    cout << "CSlowDriveModule_MAX_ROT: " << m_MaxRotation << endl;
      ...
      float trans_0, float rot_0, float rot_1)
  {
    ...
    if (fabs(rot_1) >= 0.0 && fabs(rot_1) <= 1.0) trans_1 = LinInterpol(fabs(rot_1), 0.0, 1.0, 0.7,
        fabs(m_MaxTranslation + 0.1));

    else if (fabs(rot_1) > 1.0) trans_1 = LinInterpol(fabs(rot_1), M_PI, 1.0, 0.0, 0.7);
  ...
  CSlowBackwardDriveModule::Update()
  {
    ...
    m_ProposedRotation = SlowBackward_Curvature(dist_to_target, dist_to_trajec, alpha, -m_RoboTrans, -m_RoboRot);

    if (fabs(alpha) > M_PI_2 + 0.1) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = SlowBackward_Translation(dist_to_target, dist_to_trajec, alpha, -m_RoboTrans, -m_RoboRot,
          m_ProposedRotation);
    }
    ...
    if (dist_to_target < 0.04) {
      ...
      if ((m_StopAtTarget == false) && (dist_to_target < 1.0)) {
        ...
        if (m_ProposedRotation > 0.5) m_ProposedRotation = 0.5;
        else if (m_ProposedRotation < -0.5) m_ProposedRotation = -0.5;
        else ;
      }
