...
namespace fawkes
{
      ...
      float trans_0, float rot_0, float rot_1)
  {
    ...
    if (fabs(rot_1) >= 0.0 && fabs(rot_1) <= 0.5) trans_1 = LinInterpol(fabs(rot_1), 0.5, 0.0, 1.7,
        fabs(m_MaxTranslation + 0.05));

    else if (fabs(rot_1) > 0.5 && fabs(rot_1) <= 1.0) trans_1 = LinInterpol(fabs(rot_1), 1.0, 0.5, 1.2, 1.5);

    else if (fabs(rot_1) > 1.0 && fabs(rot_1) <= 1.8) trans_1 = LinInterpol(fabs(rot_1), 1.8, 1.0, 0.8, 1.2);

    else if (fabs(rot_1) > 1.8) trans_1 = LinInterpol(fabs(rot_1), M_PI, 1.8, 0.0, 0.8);

    else {
      loggerMedBack->log_error("CMediumBackwardDriveModule",
          "************ MEDIUM DRIVE MODES:::NOT DEFINED STATE!!!!! EXITING\n");
      trans_1 = 0;
    }
   ...
   */
  void
  CMediumBackwardDriveModule::Update()
  {
    ...
    m_ProposedRotation = MediumBackward_Curvature(dist_to_target, dist_to_trajec, alpha, -m_RoboTrans, -m_RoboRot);

    if (fabs(alpha) > M_PI_2 + 0.2) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = MediumBackward_Translation(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot,
          m_ProposedRotation);
    }
    ...
    if (dist_to_target < 0.04) {
      ...
      if (m_ProposedRotation > m_MaxRotation) m_ProposedRotation = m_MaxRotation;
      ...
      if (m_StopAtTarget == false && dist_to_target < 1.5) {
        ...
        if (m_ProposedRotation > 0.6) m_ProposedRotation = 0.6;
        else if (m_ProposedRotation < -0.6) m_ProposedRotation = -0.6;
        else ;
      }
