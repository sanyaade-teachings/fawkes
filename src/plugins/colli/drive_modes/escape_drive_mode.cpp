...
//  confFileName = "../cfg/navigation/robot_shape.cfg";
    ...
    if (!config->exists("/plugins/colli/CEscapeDriveModule/EscapeForwardDriveModule_MAX_TRANS")) {
      cout << "***** ERROR *****: Could not find: EscapeForwardDriveModule_MAX_TRANS " << " --> ABORTING!" << endl
          << endl;
      return;
    } else {
    ...
    if (!config->exists("/plugins/colli/CEscapeDriveModule/CEscapeForwardDriveModule_MAX_ROT")) {
      cout << "***** ERROR *****: Could not find: CEscapeForwardDriveModule_MAX_ROT " << " --> ABORTING!" << endl
          << endl;
      return;
    }
  ...
  CEscapeDriveModule::Update()
  {
    ...
    if (dangerFront) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN FRONT\n");

    if (dangerBack) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN BACK\n");
    if (dangerLeft) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN LEFT\n");
    if (dangerRight) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN RIGHT\n");
    if (!turnLeftAllowed) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN TURNING LEFT!!!\n");
    if (!turnRightAllowed) loggerEscape->log_error("CEscapeDriveModule", "DANGER IN TURNING RIGHT !!!\n");
  ...
  CEscapeDriveModule::SortNormalizedReadings()
  {
    ...
    while (i < m_pLaser->GetNumberOfReadings()) {
      if ((pipe == 0) && !m_pLaser->IsPipe(rad)) m_vFront.push_back(m_vNormalizedReadings[i]);

      else if ((pipe == 1) && !m_pLaser->IsPipe(rad) && (rad < M_PI_2)) m_vRightFront.push_back(
          m_vNormalizedReadings[i]);
      else if ((pipe == 1) && !m_pLaser->IsPipe(rad) && (rad > M_PI_2)) m_vRightBack.push_back(
          m_vNormalizedReadings[i]);

      else if ((pipe == 2) && !m_pLaser->IsPipe(rad)) m_vBack.push_back(m_vNormalizedReadings[i]);

      else if ((pipe == 3) && !m_pLaser->IsPipe(rad) && (rad > 3 * M_PI_2)) m_vLeftFront.push_back(
          m_vNormalizedReadings[i]);
      else if ((pipe == 3) && !m_pLaser->IsPipe(rad) && (rad < 3 * M_PI_2)) m_vLeftBack.push_back(
          m_vNormalizedReadings[i]);

      else if ((pipe == 4) && !m_pLaser->IsPipe(rad)) m_vFront.push_back(m_vNormalizedReadings[i]);
  ...
  CEscapeDriveModule::TurnLeftAllowed()
  {
    for (unsigned int i = 0; i < m_vFront.size(); i++)
      if (m_vFront[i] < 0.12) return false;

    for (unsigned int i = 0; i < m_vRightFront.size(); i++)
      if (m_vRightFront[i] < 0.06) return false;

    for (unsigned int i = 0; i < m_vBack.size(); i++)
      if (m_vBack[i] < 0.07) return false;

    for (unsigned int i = 0; i < m_vLeftBack.size(); i++)
      if (m_vLeftBack[i] < 0.13) return false;
  ...
  CEscapeDriveModule::TurnRightAllowed()
  {
    ...
    for (unsigned int i = 0; i < m_vLeftFront.size(); i++)
      if (m_vLeftFront[i] < 0.06) return false;

    for (unsigned int i = 0; i < m_vBack.size(); i++)
      if (m_vBack[i] < 0.07) return false;

    for (unsigned int i = 0; i < m_vRightBack.size(); i++)
      if (m_vRightBack[i] < 0.13) return false;
