...
namespace fawkes
{
  ...
  public:
     ...
     */
    void
    SetCurrentTarget(float targetX, float targetY, float targetOri);

    /** Sets the current robo position.
     *  Has to be set before Update!
     */
    void
    SetCurrentRoboPos(float roboX, float roboY, float roboOri);
     ...
     */
    void
    SetCurrentRoboSpeed(float roboTrans, float roboRot);

    /** Set the colli mode values for each drive mode.
     *  Has to be set before Update!
     */
    void
    SetCurrentColliMode(bool orient, bool stop);
  ...
  protected:
    ...
    float
    LinInterpol(float x, float left, float right, float bot, float top);

    float
    GuaranteeTransStop(float distance, float current_trans, float desired_trans);
  ...
  inline
  CAbstractDriveMode::CAbstractDriveMode(Logger *logger, Configuration *config)
  {
    loggerDrive = logger;
    ...
    if (!config->exists("/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_TRANS_DEC")) {
      std::cout << "***** ERROR *****: Could not find: CQuadraticMotorInstruct_BASIC_TRANS_DEC " << " --> ABORTING!"
          << std::endl << std::endl;

    } else {
      if (m_sHostname == "hector_rc.informatik.rwth-aachen.de") {
        m_cMaxTransDec = config->get_float(
            "/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_TRANS_DEC");
      } else {
        m_cMaxTransDec = 0.75
            * config->get_float("/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_TRANS_DEC");
      }
    ...
    if (!config->exists("/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_ROT_DEC")) {
      std::cout << "***** ERROR *****: Could not find:  CQuadraticMotorInstruct_BASIC_ROT_DEC " << " --> ABORTING!"
          << std::endl << std::endl;

    } else {
      if (m_sHostname == "hector_rc.informatik.rwth-aachen.de") {
        m_cMaxRotDec = config->get_float(
            "/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_ROT_DEC");
      } else {
        m_cMaxRotDec = 0.75
            * config->get_float("/plugins/colli/CQuadraticMotorInstruct/CQuadraticMotorInstruct_BASIC_ROT_DEC");
      }
  ...
  inline float
  CAbstractDriveMode::LinInterpol(float x, float x1, float x2, float y1, float y2)
  {
    return (((x - x1) * (y2 - y1)) / (x2 - x1) + y1);
  }

  inline float
  CAbstractDriveMode::GuaranteeTransStop(float distance, float current_trans, float desired_trans)
  {
    ...
    if (current_trans < 0.05) return desired_trans;
    ...
    while (tmp_trans < desired_trans) {
      ++time_needed_to_stop;
      if (m_sHostname == "hector_rc.informatik.rwth-aachen.de") tmp_trans += 1.5 * m_cMaxTransDec;
      else tmp_trans += m_cMaxTransDec;
    }
