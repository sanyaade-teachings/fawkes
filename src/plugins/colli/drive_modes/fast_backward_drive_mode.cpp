...
namespace fawkes
{
      ...
      float trans_0, float rot_0, float rot_1)
  {
    ...
    if (fabs(rot_1) >= 0.0 && fabs(rot_1) <= 0.4) {
      ...
      loggerFastBack->log_error("CFastBackwardDriveModule",
  ...
  CFastBackwardDriveModule::Update()
  {
    ...
    m_ProposedRotation = FastBackward_Curvature(dist_to_target, dist_to_trajec, alpha, -m_RoboTrans, -m_RoboRot);

    if (fabs(alpha) > M_PI_2) {
      m_ProposedTranslation = 0.0;
    } else {
      m_ProposedTranslation = FastBackward_Translation(dist_to_target, dist_to_trajec, alpha, m_RoboTrans, m_RoboRot,
          m_ProposedRotation);
    }
    ...
    if (dist_to_target < 0.1) {
      ...
      if (m_StopAtTarget == false && dist_to_target < 1.0) {
        ...
        if (m_ProposedRotation > 0.3) m_ProposedRotation = 0.3;
        else if (m_ProposedRotation < -0.3) m_ProposedRotation = -0.3;
        else ;
      }
    }
  }
