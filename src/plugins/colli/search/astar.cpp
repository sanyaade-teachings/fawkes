...
namespace fawkes
{
  ...
  ColliAStar::ColliAStar(Logger* logger, Configuration *config, OccupancyGrid * occGrid)
  {
    ...
    if (!config->exists("/plugins/colli/AStar_MAX_STATES")) {
      cout << "***** ERROR *****: Could not find: AStar_MAX_STATES" << " --> ABORTING!" << endl << endl;
      exit(0);
    } else {
   ...
   */
  void
  ColliAStar::Solve(const HomPoint &RoboPos, const HomPoint &TargetPos, vector<HomPoint> &solution)
  {
    seen_states.clear();
    occ_cells.clear();
   ...
   */
  int
  ColliAStar::CalculateKey(int x, int y)
  {
   ...
   */
  void
  ColliAStar::GenerateChildren(ColliAStarState * father)
  {
   ...
   */
  int
  ColliAStar::Heuristic(ColliAStarState * state)
  {
    //  return (int)( abs( state->m_X - m_pTargetState.m_X ));
    return (int) (abs(state->m_X - m_pTargetState.m_X) + abs(state->m_Y - m_pTargetState.m_Y));
  }
   ...
   */
  bool
  ColliAStar::IsGoal(ColliAStarState * state)
  {
    return ((m_pTargetState.m_X == state->m_X) && (m_pTargetState.m_Y == state->m_Y));
  }
   ...
   */
  void
  ColliAStar::GetSolutionSequence(ColliAStarState * node, vector<HomPoint> &solution)
  {
  ...
  HomPoint
  ColliAStar::RemoveTargetFromObstacle(int targetX, int targetY, int stepX, int stepY)
  {
    // initializing lists...
    while (m_pOpenList.size() > 0)
      m_pOpenList.pop();
    ...
    while (!(m_pOpenList.empty()) && (m_AStarStateCount < m_MaxStates - 6)) {
      ...
      if (m_hClosedList.find(key) == m_hClosedList.end()) {
        ...
        if ((father->m_X > 1) && (father->m_X < (signed) m_Width - 2)) {
          ...
          } else if (m_hClosedList.find(key) == m_hClosedList.end()) m_pOpenList.push(child);
        }
