...
namespace fawkes
{
  ...
  public:
     ...
     */
    float
    GetSin(float radians);

    /** return linear interpolated cosinus.
     */
    float
    GetCos(float radians);
  ...
  private:
    ...
    float
    InterpolateSin(float index_to_interpolate);
    float
    InterpolateCos(float index_to_interpolate);
    int m_Resolution;
  };
  ...
  TrigTable::GetSin(float radians)
  {
    ...
    if (index - degree == 0) return m_vSinTable[index];
    else return InterpolateSin(degree);
  }
  ...
  TrigTable::GetCos(float radians)
  {
    ...
    float degree = rad2deg(radians) * m_Resolution;
    int index = (int) degree;
    if (index - degree == 0) return m_vCosTable[index];
    else return InterpolateCos(degree);
