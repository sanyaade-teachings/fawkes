...
TabletopVisualizationThread::loop()
{
  ...
  for (M_Vector4f::const_iterator it = centroids_.cbegin(); it != centroids_.cend(); it++) {
    try {
      tf::Stamped<tf::Point>
      ...
      char *tmp;
      if (asprintf(&tmp, "TObj %zu", static_cast<size_t>(it->first)) != -1) {
      ...
      sphere.scale.z = 0.09;
      sphere.color.r = (float)cluster_colors[it->first][0] / 255.f;
      sphere.color.g = (float)cluster_colors[it->first][1] / 255.f;
      sphere.color.b = (float)cluster_colors[it->first][2] / 255.f;
      sphere.color.a = 1.0;
